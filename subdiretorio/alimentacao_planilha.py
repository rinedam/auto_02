...
def processar_arquivo_swwweb(download_folder,planilha_destino, planilha_folder):
    ...
    if ultimo_arquivo:
        ...
        excel_window.type_keys('^{HOME}', with_spaces=True)  # Move para célula A1
        time.sleep(1)
        excel_window.type_keys('^+{END}', with_spaces=True)  # Ctrl+Shift+End seleciona tudo até o final
        time.sleep(1)
        excel_window.type_keys('{DELETE}', with_spaces=True)  # Delete para apagar
        time.sleep(1)
        excel_window.type_keys('^{HOME}', with_spaces=True)  # Move para célula A1
        time.sleep(1)
